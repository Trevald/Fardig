...
export const MentionTagsMixin = {
	...
	methods: {
		mentionTagsUpHandler() {
			this.mentionTagsNavigatedTagIndex =
				(this.mentionTagsNavigatedTagIndex + this.mentionTagsFilteredTags.length - 1) %
				this.mentionTagsFilteredTags.length
		},

		mentionTagsDownHandler() {
			this.mentionTagsNavigatedTagIndex =
				(this.mentionTagsNavigatedTagIndex + 1) % this.mentionTagsFilteredTags.length
		},

		mentionTagsEnterHandler(query) {
			const tag = this.mentionTagsFilteredTags[this.mentionTagsNavigatedTagIndex]
			console.log(
				"mentionTagsEnterHandler",
				this.mentionTagsFilteredTags,
				this.mentionTagsNavigatedTagIndex
			)
			if (tag) {
				this.mentionTagsSelectTag(tag)
			} else {
				this.mentionTagsSelectTag(query)
			}
		},
